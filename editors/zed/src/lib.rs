...
impl JarlExtension {
    fn language_server_binary(
        ...
        // Use user specified path to the `jarl` binary, if it is specified
        if let Some(path) = binary_settings.and_then(|binary_settings| binary_settings.path) {
            return Ok(JarlBinary { path, args: binary_args });
        }

        // Use binary on the `PATH`, if it exists
        if let Some(path) = worktree.which("jarl") {
            return Ok(JarlBinary { path, args: binary_args });
        }
        ...
        if let Some(path) = &self.cached_binary_path {
            if fs::metadata(path).is_ok_and(|stat| stat.is_file()) {
                return Ok(JarlBinary { path: path.clone(), args: binary_args });
            }
        }
...
impl GithubReleaseDetails {
    fn new(
        ...
        let downloaded_directory = format!("jarl-{version}");

        let downloaded_binary_path = match platform {
            zed::Os::Mac | zed::Os::Linux => format!("{downloaded_directory}/{asset_stem}/jarl"),
            zed::Os::Windows => format!("{downloaded_directory}/jarl.exe"),
        };
...
impl zed::Extension for JarlExtension {
    fn new() -> Self {
...
zed::register_extension!(JarlExtension);
    ...
    fn test_github_release_details() {
        assert_eq!(
            ...
            GithubReleaseDetails {
                asset_name: String::from("jarl-aarch64-apple-darwin.tar.gz"),
                downloaded_file_type: zed_extension_api::DownloadedFileType::GzipTar,
                downloaded_directory: String::from("jarl-0.1.0"),
                downloaded_binary_path: String::from("jarl-0.1.0/jarl-aarch64-apple-darwin/jarl")
            }
        );
        ...
        assert_eq!(
            ...
            GithubReleaseDetails {
                asset_name: String::from("jarl-x86_64-unknown-linux-gnu.tar.gz"),
                downloaded_file_type: zed_extension_api::DownloadedFileType::GzipTar,
                downloaded_directory: String::from("jarl-0.2.0"),
                downloaded_binary_path: String::from(
                    "jarl-0.2.0/jarl-x86_64-unknown-linux-gnu/jarl"
                )
            }
        );
        ...
        assert_eq!(
            ...
            GithubReleaseDetails {
                asset_name: String::from("jarl-x86-pc-windows-msvc.zip"),
                downloaded_file_type: zed_extension_api::DownloadedFileType::Zip,
                downloaded_directory: String::from("jarl-0.1.0"),
                downloaded_binary_path: String::from("jarl-0.1.0/jarl.exe")
            }
        );
    }
}
